...
func (s usercentApp_local_stub) RegisterRouter(ctx context.Context) (err error) {
	span := trace.SpanFromContext(ctx)
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.tracer.Start(ctx, "usercentApp.UsercentApp.RegisterRouter", trace.WithSpanKind(trace.SpanKindInternal))
		defer func() {
			if err != nil {
				span.RecordError(err)
				span.SetStatus(codes.Error, err.Error())
...
func (s usercentApp_client_stub) RegisterRouter(ctx context.Context) (err error) {
	// Update metrics.
	start := time.Now()
	...
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.stub.Tracer().Start(ctx, "usercentApp.UsercentApp.RegisterRouter", trace.WithSpanKind(trace.SpanKindClient))
	}
...
func (s usercentApp_server_stub) registerRouter(ctx context.Context, args []byte) (res []byte, err error) {
	// Catch and return any panics detected during encoding/decoding/rpc.
	defer func() {
		if err == nil {
			err = codegen.CatchPanics(recover())
