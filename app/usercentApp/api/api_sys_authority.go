...
func (a *AuthorityInfoApi) DeleteAuthority(w http.ResponseWriter, r *http.Request) {
	...
	err := common.HttpRequest2Struct(r, &auth)
	if err != nil {
		common.HttpOKErrorResponse(w, errorcode.ErrUserComm.FillMsg(err.Error()))
		return
	}

	if err := service.AuthorityServices.DeleteAuthority(&auth); err != nil { // 删除角色之前需要判断是否有用户正在使用此角色
...
func (a *AuthorityInfoApi) UpdateAuthority(w http.ResponseWriter, r *http.Request) {
	...
	if authority, err := service.AuthorityServices.UpdateAuthority(auth); err != nil {
		common.HttpOKErrorResponse(w, errorcode.ErrUserComm.FillMsg("更新失败-"+err.Error()))
	} else {
		common.HttpOKResponse(w, response.SysAuthorityResponse{Authority: authority})
	}
}
